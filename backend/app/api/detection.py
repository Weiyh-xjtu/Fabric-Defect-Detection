...
async def detect_video_api(
    ...
    conf: float = Form(0.25, description="置信度阈值"),
    frame_sample_rate: int = Form(5, ge=1, description="帧采样间隔（每 N 帧取 1 帧）"),
    max_frames: int = Form(50, ge=1, description="最多处理的关键帧数量"),
    scene_id: int = Form(None, description="场景 ID"),
    current_user=Depends(get_current_user),
):
    """
    ...
    # ── 保存视频到临时文件，并在服务端限制为 50MB ──
    # 不能只依赖前端校验；同时分块读取可避免一次性把大文件放入内存。
    tmp_path = None
    file_size = 0
    try:
        with tempfile.NamedTemporaryFile(suffix=suffix, delete=False) as tmp:
            ...
            while chunk := await file.read(1024 * 1024):
                ...
                if file_size > MAX_VIDEO_SIZE_BYTES:
                    raise ValueError("视频文件不能超过 50MB")
                tmp.write(chunk)
    ...
    # ── 先创建检测任务记录 ──
    resolved_scene_id = scene_id or 1
    user_id = current_user.id
    db = SessionLocal()
    try:
        task = DetectionTask(
