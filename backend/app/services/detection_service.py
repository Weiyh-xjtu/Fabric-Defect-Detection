...
class DetectionService:
    ...
    def detect_video(
        ...
        try:
            ...
            # ── 逐帧处理：按采样帧执行推理，其他帧复用最近一次结果 ──
            # `sample_indices` 已按照 frame_sample_rate / max_frames 计算；必须
            # 用它控制推理次数，否则较长视频会逐帧推理，max_frames 也会失效。
            ...
            while True:
                ...
                if frame_idx in sample_set:
                    ...
                    logger.debug(
                        "视频检测进度: 已处理 %d/%d 个采样帧，当前帧号 %d，检测到 %d 个目标",
                        sampled_count,
                        len(sample_indices),
                        frame_idx,
...
def get_model(scene_id: int = None) -> YOLO:
    """供视频摄像头等外部入口复用的模型加载函数。"""
    return detection_service._get_model(scene_id)
